...
function makeSpatialKey(cs, {x,y,z,width,height,depth}){
    // var keykey = `${cs},${x},${y},${z},${width},${height},${depth}`
    // var keykey = BigInt(x+y*1000+z*1000000)+BigInt(width*1000000+height*1000+depth)*10000000n
    // var memres = keycache.get(keykey)
    // if (memres) {
        // return memres
    // }
    // var count = 0;
    ...
    // var keys = new Int32Array(4+Math.ceil(((x%cs)/cs))+Math.ceil(((y%cs)/cs))+Math.ceil(((z%cs)/cs))+Math.floor(width/cs)+Math.floor(height/cs)+Math.floor(depth/cs))
    var keys = []
