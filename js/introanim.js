...
export async function doIntroAnim(pokitOS) {
    ...
    return new Promise((resolve) => text.addUniqueCog('doanim', { update: () => {
            ...
            else if (bottombar.x > 0) {
                bottombar.x -= animrate * 4;
            }
            else if (text.width * text.scaleX < 320 * 32) {
                [text, topbar, bottombar].forEach(x => {
                    x.scaleX += mag * 0.1;
                    x.scaleY += mag * 0.1;
                    // x.rotation += mag * .5 
                    mag += 0.03;
